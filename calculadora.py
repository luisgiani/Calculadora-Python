...
def main(page: ft.Page):
    ...
    def select(e):
        ...
        if value == 'AC':
            ...
            if valor_atual == '0' or valor_atual == 'Erro':
                resultado.value = value
            else:
                resultado.value = valor_atual + value
        
        elif value in ['+', '-', '*', '/', '.']:
            if valor_atual == 'Erro':
                resultado.value = '0' + value
            elif valor_atual[-1] in ['+', '-', '*', '/', '.']:
                resultado.value = valor_atual[:-1] + value
            else:
                resultado.value = valor_atual + value
        
        elif value in ['=', '%', '±']:
            if valor_atual != 'Erro':
                resultado.value = calculate(operador=value, valor_atual=valor_atual)
